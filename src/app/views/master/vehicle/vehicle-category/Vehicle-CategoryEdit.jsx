...
const VehicleCategoryEdit = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    password: "",
    confirmPassword: "",
    VehicleCategory_option: null // ✅ new field for autocomplete
  });

  useEffect(() => {
    fetch(`/api/VehicleCategoryes/${id}`)
      .then((response) => response.json())
      .then((data) => {
        // Match suggestion based on value
        const selectedOption = suggestions.find((opt) => opt.value === data.VehicleCategory_option);
        setFormData({ ...data, VehicleCategory_option: selectedOption || null });
      });
  }, [id]);
  ...
  const handleSubmit = (e) => {
    ...
    const payload = {
      ...formData,
      VehicleCategory_option: formData.VehicleCategory_option?.value || "" // Just value
    };
    ...
    }).then(() => navigate("/master/VehicleCategory"));
  };

  return (
    <div>
      <h2>Edit VehicleCategory</h2>
        ...
        <Grid item md={6} xs={12} lg={3}>
          ...
          <Grid item md={6} xs={12} lg={3}>
            <Autocomplete
              ...
              value={formData.VehicleCategory_option}
              onChange={(event, newValue) => setFormData({ ...formData, VehicleCategory_option: newValue })}
              renderInput={(params) => (
                <TextField
                  {...params}
                  label="Select Status"
